...
The transfer also stops with an error issue if a source adapter repeats any cursor, which
prevents a broken paging implementation from causing an infinite loop or duplicate copy.
